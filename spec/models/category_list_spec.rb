...
describe CategoryList do
  ...
  context "security" do
    it "properly hides categories from users who aren't admins and aren't in the icij project" do
      ...
      category = Fabricate(:private_category, group: icij_group)
      Fabricate(:topic, category: category)
      category.set_permissions(icij_group.id => 1)
      category.save
    ...
    # this whole test is mal-conceived and needs to be re-written
    ...
    #   expect(CategoryList.new(Guardian.new(user), include_topics: true).categories.find { |x| x.name == private_cat.name }.displayable_topics.count).to eq(1)
    #
