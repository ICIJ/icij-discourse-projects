...
describe UserGuardian do
  ...
  describe "#can_see_profile?" do
    ...
    it "is false for a user whose profile is public" do
      expect(Guardian.new.can_see_profile?(user)).to eq(false)
    end

    context "current_user is in same ICIJ project as viewed user" do
      let(:other_user) do
        Fabricate(:user, id: 5)
      end
      ...
      it "is true" do
        icij_group.users << other_user
        icij_group.users << user

        expect(Guardian.new(other_user).can_see_profile?(user)).to eq(true)
      end
    end

    context "current_user is not in same ICIJ project as viewed user" do
      let(:other_user) do
        Fabricate(:user, id: 6)
      end
      ...
      it "is true" do
        ...
        expect(Guardian.new(other_user).can_see_profile?(user)).to eq(false)
      end
    end
